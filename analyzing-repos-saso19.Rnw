...
self-organization, with code growing and evolving mainly through activities that do not stem from centralized power, leader or directives. Although there is anecdotal evidene for this,  self organization
cannot be detected through direct observation, but through measurements on the system,
looking for features such as the existence of powerlaws over some
features (such as changes) and sizes. Although published and proposed in many papers as such, power laws are more elusive than they might seem at first sight, so in this paper we present a statistically accurate
set of tests that help us decide, from the way repositories are
changing, if they are really distributed by a powerlaw, indicating the
existence of a state reached via self-organization.
...
Understanding what distribution is producing our data gives us insights about 
how confidently we are about claiming that our system is a self-organized one.
However, during this process, we can show a distorted
version of reality if aren't careful enough. % Clarify what you mean by "A distorted version of reality", saying clearly what you want: revisit claims for the existence of power laws, showing that in most cases they only seem a power law. - JJ
...
Up to this point, we have ruled out what datasets are unlikely to be fitted by a powerlaw
in any of its distribution. Notice, on the contrary case, that we are only assuming that
...
% Can we maybe show all repos in the past and now? Would that be useful?
