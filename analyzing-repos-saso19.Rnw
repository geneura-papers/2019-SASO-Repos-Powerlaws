...
\begin{abstract}
Software development is an example of self-organized systems. Even if
there can be team leaders, the growth and evolution of the code in
repositories is mainly the result of self-organization. Self
organization cannot be deteted directly, but through measurements in
the system, looking for features such as the existence of a
powerlaw. In this paper we present a statistically accurate set of
tests that help us decide how repositories are evolving, and if they
are really distributed by a powerlaw, indicating the existence of a
state reached via self-organization.
...
There are several steps in the study of self-organized systems. One of
them is to study the existence of powerlaw's distribution in the data
from our system. % Add reference. Always add references. - JJ
However, during this process, we can show a distorted
version of reality if aren't careful enough. 

To really understand the behaviour of software development teams as
reflected by their activity in repositories, we should start to not
...
behaviour. Since this kind of state need not be steady, we should need
to analyze it not only in a  point in time, but also taking into
account the temporal dimension.
% In general, you shouldn't say what we are about to do. We should say
% the motivation for doing this kind of thing, and also qualify or
% reference *any* claim we do. In this case
% * Why is self-organization in repositories important? What would it imply
% * Why does this advance the state of the art? Is it because, in this
% particular case, power laws are not thoroughly investigated?

This goal leads us to study where a possible powerlaw behaviour would
start, since a pure, full-range powerlaw distribution is far form
usual. % reference needed - JJ 
Moreover, we should estimate the scale of our distribution and the
error we are make in such estimation. % why? - JJ
To end this part, we present an interesting comparative study, where
we observe the invariance of scale and global behaviour in time. % Why? - JJ

But this study cannot be completed without a proper distribution
analysis. % It's not that it wouldn't be completed. It's that we need
          % to know which distribution is the most likely, if it's not
          % a power law. Also, why is it mistaken by a power law. Some
          % introduction to statistical distribution is not in
          % order. - JJ
In some datasets, the evidence of a powelaw is very weak, and
even in the ones that pass our entry test, we cannot afirm blindly
that powerlaws are generating them. So,  In order to minimize our
preexisting bias, we are going to measure the evidence of a powerlaw
distribution in front of a series of distributions that could lead to
similar results. % These are conclusions, and do not belong
                 % here. Questions belong to the introduction. Is
                 % there evidence of a power law? What are we going to
                 % do to establish that evidence? How does it prove
                 % our original hypothesis or not? - JJ

% Lay out the skeleton of what we are going to do. We will re-measure
% what we did in our 2017 paper, see if it holds (justify why we use
% this data). Also say what kind of data we are using, how we obtain
% it... Introduction should take the first full page, roughly.


This way, we offer a test to analyze how much evidence there is for or
against our assumptions in every step, finally addressing the
question: Does software development really follows a powerlaw? 
% This is a small sample of all software development. You will need to
% justify this choice later on.
...
goodness-of-fit  test via boostrapping procedure. 
...
and $\alpha$  and  then  we reinferr the model parameters. We perfom this analysis with 
R package: PoweRlaw. A detailed description of it and the hypothesis tests can be found 
in \cite{gillespie2015power}.

From our hypothesis test , there are two possible outcomes. 
If our p-value$<0.1$, then we should search for another model that fit our
data better than the powerlaw.
On the other hand, if our p-value $> 0.1$
we can not rule outthe power law model. Then, we can deeply analyze its parameters and evolution.
...
least square method is a poor but widespread way to proceed when estimating the
