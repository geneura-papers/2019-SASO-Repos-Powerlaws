...
There are several steps in the study of self-organized systems  \citep{bak1988self}. One of
them is to study the existence of powerlaw's distribution in the data
from our system. This property is intrinsic related to the scale-free behaviour
these systems exhibit, that is, there is no single characteristic event size \cite{golyk20self}.
% Add reference. Always add references. - JJ
Understanding what distribution is producing our data give us an insight about 
how confidently we are about confirm that our system is a Self-organized one.
...
easily be confounded by a powerlaw. This is an standard way to discover if our data is more likely to be produced by other heavy-tailed distribution \cite{clauset2009power}, like the logNormal or the truncated powerlaw. 

Finally, since we are able to access to multiple years' data, we also interested in study the temporal evolution of the two previous analysis. Here our main objetive is to show how these measure could 
have changed (if they have at all). We should remember that repositories are evolving system by definition, so they can experiment qualitative changes in his events distribution. Furthermore, even 
...
of these system, specially in the presence of phase changes \cite{merelo2017self} or self-organized critically (understood as a mechanism by which the system is regulating himself to stay in those parameters \cite{newman2005power}).
...
changes, establishing a statiscally correct starting point to answer the question: Does software development really follows a powerlaw? 
