...
phase change, and, for a phase change to happen, sooner or later, we will cross a momentary critical state. % I don't like how this sentence is built... Too roundabout - JJ
...
% You normally write a small introduction to every section. You can't
% just set the first subsection after the section header - JJ
...
probably different from the 2017's fit  % Implying what? - JJ 
%
\begin{table*}[h!tbp]
...
distribution fit of the repository in 2019 has caused a little controversy
...
by their algorithms. One package gives more probability of fitting a power law between 10 to $10^5$ commit size  and 
the other starting from $10^5$ commit size till the end. However, choosing different starting points produced similar results in the final tests. % "has been" or "has caused"? What is the nature
                      % of that controversy? - JJ

After analyzing the parameters estimated by assuming a power law fit, is time to check what are  %analyzing the parameter
                                %analysis? Really? - JJ 
the results from the hypothesis
tests, made to know if there are other possible distributions generating the data. These results can be found in Tables \ref{tab:2017tests} and
\ref{tab:2019tests}.  
...
% No repository with "high" rank? - JJ
% No, maximum rank achieved is moderate, that is, PL is not discard but there are other that could fit too
\begin{figure*}[htbp]
...
\begin{figure*}[htbp]
	...
	\caption{PDF plot of all repositories 2017. Red line
          represents the power law fit and the area where it fits.}
	\label{fig:pl2017}
\end{figure*}
...
\begin{figure*}[htbp]
	...
	\caption{PDF plot of all repositories 2019. Red line
          represents the power law fit and the area where it actually fits.}
	\label{fig:pl2019}
\end{figure*}
...
order to affirm any behaviour in our data's distribution. That way, we
can assure that our results are unbiased (not based in our sight or
plots) and they have been tested statistically. This is what we have
done in this paper, that not only uses that on data already published,
...
In general, the probability of finding a power law  in the
repositories studied is quite low, so, up to now, we can not affirm
categorically that the studied systems are in a critical state due to
self-organization. 
...
they can be related to a possible phase change. % But is there a power
                                % law? - JJ
...
On the other hand, as our sample size is relatively small, we could
...
We would like to conclude pointing out that all the software used in this research (being the main ones R\cite{R}, Python \cite{CS-R9526} and \LaTeX) is open source. Furthermore, all the scripts used and data obtained (curated and not curated) are open and can be checked at \url{https://github.com/geneura-papers/2019-SASO-Repos-Powerlaws}.
% Mention software and scripts used, and that they are open source - JJ
