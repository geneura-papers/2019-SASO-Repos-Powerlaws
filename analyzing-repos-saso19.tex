...
\author{\IEEEauthorblockN{1\textsuperscript{st} Bartolom\'{e} Ortiz}
  \IEEEauthorblockA{\textit{Geneura Team,ETSIIT and CITIC} \\
    ...
    Granada, Spain \\
    bortiz@ugr.es} \and \IEEEauthorblockN{2\textsuperscript{nd}
    J. J. Merelo}
  \IEEEauthorblockA{\textit{Geneura Team,ETSIIT and CITIC} \\
    \textit{University of Granada}\\
    Granada, Spain \\
...
\begin{abstract}
  ...
  existence of powerlaws over some features, such as the size of
...
power-law's distribution in the data from our system. This property is
...
critical
state. % I don't like how this sentence is built... Too roundabout - JJ
...
tests. % "has been" or "has caused"? What is the nature
% of that controversy? - JJ

After analyzing the parameters estimated by assuming a power law fit,
is time to check what are %analyzing the parameter
% analysis? Really? - JJ
...
studied is quite low, so, up to now, we can not affirm categorically
...
Thanks to this procedure, we hope we can constitute a new state of
...
author, per file, blame chunks \footnote{Code lines that are next to
...
% Mention software and scripts used, and that they are open source -
% JJ

\bibliographystyle{apalike} \bibliography{geneura,biblio}
